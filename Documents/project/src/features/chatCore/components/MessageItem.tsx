...
const MessageItem: React.FC<MessageItemProps> = ({ 
  ...
  return (
    <div className={containerClasses + " isolate"}>
      ...
      <div className={messageClasses + " relative"}>
                  ...
                  {message.file_urls.map((url: string, idx: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex flex-col flex-grow min-w-0">
                          <span className="font-semibold text-[14px] text-[#1D2A4D] dark:text-white truncate" title={fileName}>{fileName}</span>
                        </div>
                        <a
                          href={url}
                          target="_blank"
        ...
        <div className="mt-2 flex flex-row items-center gap-4 text-[12px] text-[#3A3A3A] dark:text-gray-400 font-normal">
          <div className="flex items-center gap-1 cursor-pointer select-none" onClick={handleCopy}>
            <span className={`inline-flex items-center gap-0.5 px-1 py-0.5 rounded transition-colors duration-150 ${copied ? 'bg-[#00CABA] text-white' : 'hover:bg-[#F5F7FA] text-[#3A3A3A] hover:text-[#00CABA]'}`}
                  style={{ fontWeight: 500, fontSize: '12px', textTransform: 'none', letterSpacing: '0.02em' }}>
