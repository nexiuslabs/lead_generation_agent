...
const PreviewPanel: React.FC<PreviewPanelProps> = ({
  ...
  const handleSendReply = () => {
    if (onSendReply) onSendReply(replyDraft);
    // Optionally clear draft or show feedback
    // setReplyDraft('');
  };
    ...
    >
      ...
      <nav className="flex px-3 sm:px-6 pt-2 pb-0 border-b border-gray-100 dark:border-gray-800 overflow-x-auto" aria-label="Tabs">
        {tabList.map((tab) => (
          <button
            key={tab.key}
            className={`mr-6 sm:mr-8 pb-2 text-sm sm:text-base font-semibold focus:outline-none transition-colors
              ${activeTab === tab.key
